...
class BlogPostTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.post = mixer.blend(models.BlogPost,
                                ...
                                slug=mixer.SKIP,
                                thumbnail=mixer.RANDOM)

    def test_delete_category_w_posts(self):
        with self.assertRaises(
                ProtectedError,
                msg="You shouldn't be able to delete a category "
                    "with posts in it"):
    ...
    def test_slug(self):
        ...
        self.post.save()
        assert self.post.slug == 'django-30-tdd-in-a-nutshell', \
            'Slug should be attached automatically'
...
class IllustrationTestCase(TestCase):
    ...
    def test_upload_illustration(self):
        ...
        assert storage.exists(img_name), \
            'File must be uploaded into MEDIA_ROOT directory'
    ...
    def test_assign_illustration(self):
        ...
        assert illustration == post.illustrations.first(), \
            'Illustration must be in the database'

        storage = FileSystemStorage()
        img_filename = os.path.basename(illustration.picture.name)
        assert not storage.exists(f'{settings.MEDIA_URL}tmp/{img_filename}'), \
            'Image must be moved from tmp folder after saving'
    ...
    def test_steal_illustration(self):
        ...
        with self.assertRaises(SuspiciousFileOperation):
            models.Illustration.assign_illustration(illustration.picture.url,
                                                    post2)
...
class CleanupTestCase(TestCase):
    ...
    def test_illustrations_cleanup_when_post_deleted(self):
        ...
        img_storenames = [illustration.picture.name for illustration
                          in illustrations]
        img_storenames.append(post.thumbnail.name)
        self.assertMediaCleanedUp(post, *img_storenames)
